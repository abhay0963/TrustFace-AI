...
"what happens, in order" narrative. Architecture first, then backend file
by file (where nearly all the logic lives), then frontend.
...
┌─────────────────────────────────────────────────────────────────┐
│  BROWSER (client)                                                │
│  Vanilla JS · Jinja2-rendered HTML · Plotly · getUserMedia webcam│
└───────────────────────────┬───────────────────────────────────┬─┘
                             │ HTTP (JSON / form)                │
┌────────────────────────────▼───────────────────────────────────▼┐
│  FASTAPI APP  (backend/main.py)                                  │
│  Routes only — every handler is 1-3 lines, delegates immediately │
└───────────────────────────┬────────────────────────────────────┬┘
                             │                                    │
┌────────────────────────────▼─────────────┐   ┌──────────────────▼─┐
│  SERVICE / ORCHESTRATION LAYER            │   │  READ-ONLY QUERIES  │
│  recognition_service.py                   │   │  (dashboard, users, │
│  register_face() · recognize_frame()      │   │   logs pages call   │
└──┬─────────┬─────────┬─────────┬─────────┬┘   │   database.py /     │
   │         │         │         │         │    │   presence.py       │
┌──▼───┐ ┌───▼────┐ ┌──▼──────┐┌─▼───────┐┌▼────▼──────────────────┐│
│face_ │ │privacy │ │trust_   ││ml_trust_││presence.py              ││
│engine│ │  .py   │ │engine.py││engine.py││(duration/consistency/   ││
│  .py │ │ (AES)  │ │(rules)  ││(sklearn)││ category classification)││
└──┬───┘ └───┬────┘ └────┬────┘└────┬────┘└───────────┬─────────────┘│
   │         │            │         │                 │              │
┌──▼─────────▼────────────▼─────────▼─────────────────▼──────────────▼┐
│  DATABASE LAYER  (database.py)                                       │
│  SQLite — users · attendance_sessions · recognition_logs             │
└────────────────────────────────────────────────────────────────────┘

External, pretrained, called by face_engine.py:
┌────────────────────────────┐   ┌──────────────────────────────┐
│  InsightFace / ONNX Runtime │   │  spoof_model.py               │
│  (buffalo_l: detector +     │   │  → real ONNX model if present,│
│   ArcFace recognizer)       │   │  → else FFT heuristic fallback│
└──────────────────────────────┘   └──────────────────────────────┘
```

**The rule that makes this maintainable**: every arrow points downward
only. `main.py` never touches the database directly — it always goes
through `recognition_service.py`, `database.py`, or `presence.py`.
`recognition_service.py` never touches SQLite directly — it always goes
through `database.py`. This is why any single file can be read, tested, or
explained in isolation.

## 1.2 Folder → responsibility map

| Folder | Responsibility |
|---|---|
| `backend/` | All logic — AI pipeline, encryption, database, HTTP routes |
| `frontend/templates/` | Server-rendered page shells (Jinja2) |
| `frontend/static/` | Everything that runs in the browser — CSS, and the JS that calls the API |
| `learning_lab/` | Standalone scripts that exercise the backend modules directly (no HTTP) to teach one AI concept each |
| `experiments/` | Standalone scripts that run reproducible comparisons/ablations for the paper |
| `database/` | Generated at runtime — `trustface.db`, `secret.key` (gitignored) |
| `models/` | Generated/downloaded at runtime — InsightFace cache, trained `trust_classifier.pkl`, optional `anti_spoof.onnx` (gitignored) |
| `docs/`, `paper/` | Documentation and the research paper scaffold — no code |

## 1.3 The request lifecycle (the single most important diagram)

Every live recognition, end to end:

```
 1. Browser captures a webcam frame every 1.5s (attendance.js)
          │  POST /api/recognize  { image: base64 }
          ▼
 2. main.py: api_recognize()
          │  decode_base64_image()
          ▼
 3. recognition_service.recognize_frame(image_bgr)
          │
          ├─► face_engine.detect_faces()            → InsightFace: bbox + landmarks + 512-d embedding
          ├─► face_engine.compute_quality_metrics()  → blur, brightness, pose, face_size
          │        └─► spoof_model.get_spoof_score() → real ONNX model OR face_engine's FFT heuristic
          ├─► privacy.decrypt_embedding()             → per gallery user, IN-MEMORY ONLY
          ├─► face_engine.cosine_similarity()          → best match + similarity score
          ├─► trust_engine.compute_trust_score()        → RULE-BASED decision (always computed)
          ├─► ml_trust_engine.predict_decision()         → ML decision, IF a model has been trained
          │
          │   ┌─────────────────────────────────────────────────────┐
          │   │  AUTHORITATIVE DECISION (bootstrap-then-replace):     │
          │   │  ML model trained & promoted?  → ML decision wins     │
          │   │  Otherwise                     → rule decision wins   │
          │   └─────────────────────────────────────────────────────┘
          │
          ├─► database.insert_recognition_log()   → ALWAYS logs rule decision + ml decision + final (authoritative) decision
          └─► presence.record_presence_ping()      → ONLY if authoritative decision == "auto_accept"
          ▼
 4. JSON response → browser draws bounding boxes + animates the Trust Gauge
```

## 1.4 The Trust Engine's evolution (V1 → V2, not two permanent systems)

```
        ┌───────────────────┐         enough labeled data          ┌────────────────────┐
        │  V1: RULE ENGINE   │  ───────────────────────────────►   │  V2: ML ENGINE       │
        │  weighted sum,     │      (MIN_TRAINING_SAMPLES = 30,     │  Decision Tree /      │
        │  works from data 0 │       human-labeled via Logs page)   │  Random Forest        │
        └───────────────────┘                                      └────────────────────┘
                 │                                                            │
                 │              both ALWAYS computed and logged               │
                 └──────────────────────────┬─────────────────────────────────┘
                                             ▼
                              ONE authoritative decision drives
                              presence logging + the UI's primary badge
                              (config.ML_ENGINE_PROMOTE_WHEN_READY controls this)
```

## 1.5 Data model (entity relationships)

```
users ─────────────< attendance_sessions          users ─────────────< recognition_logs
 id (PK)                 user_id (FK)              id (PK)                 user_id (FK, nullable — unknown faces)
 name                     session_date              name                    similarity
 external_id (unique)     entry_time / last_seen    external_id             trust_score
 embedding_enc (AES)      presence_seconds                                  decision (rule) / ml_decision / final_decision
 created_at               ping_count                                        blur/brightness/pose/face_size/spoof scores
                          avg_trust_score                                   human_label (ground truth for ML training)
                          avg_confidence
```

One `attendance_sessions` row per `(user, day)` — updated in place on every
ping. One `recognition_logs` row per **attempt** — append-only, the full
audit trail and the ML training set.

## 1.6 Where the "AI" actually lives

| Component | Pretrained or trained-by-this-project? |
|---|---|
| Face detector (RetinaFace-style) | **Pretrained** — InsightFace `buffalo_l` |
| Face recognizer (ArcFace) | **Pretrained** — InsightFace `buffalo_l` |
| Rule-based Trust Engine | Hand-designed formula (not ML) |
| ML Trust Engine (Decision Tree / Random Forest) | **Trained by this project** — on your own human-labeled data, real train/test split |
| Anti-spoof | Heuristic by default; **pluggable slot** for a pretrained ONNX model |

## 1.7 Deployment architecture

Single process, single machine — no separate services to coordinate:

```
Your laptop
 ├─ uvicorn (one process) ── serves HTTP + runs InsightFace inference on CPU
 ├─ trustface.db (one SQLite file) ── no separate database server
 └─ ~/.insightface/models/, models/trust_classifier.pkl ── local disk cache
```

This is why `run.py` is the entire "ops" story — there's no container
orchestration, no message queue, no separate model-serving process to
reason about, which is exactly right for a laptop-scale MVP.
